...
HRESULT GenerateSharedToggleElements(ABI::Windows::Foundation::Collections::IVector<ABI::AdaptiveNamespace::AdaptiveToggleTarget*>* targets,
                                     std::vector<std::shared_ptr<AdaptiveSharedNamespace::ToggleVisibilityTargetElement>>& containedElements)
{
    containedElements.clear();

    ComPtr<ABI::Windows::Foundation::Collections::IVector<ABI::AdaptiveNamespace::AdaptiveToggleTarget*>> localTargets(targets);
    ComPtr<IIterable<ABI::AdaptiveNamespace::AdaptiveToggleTarget*>> vectorIterable;
    THROW_IF_FAILED(localTargets.As<IIterable<ABI::AdaptiveNamespace::AdaptiveToggleTarget*>>(&vectorIterable));

    Microsoft::WRL::ComPtr<IIterator<ABI::AdaptiveNamespace::AdaptiveToggleTarget*>> vectorIterator;
    HRESULT hr = vectorIterable->First(&vectorIterator);
    ...
    while (SUCCEEDED(hr) && hasCurrent)
    {
        ComPtr<ABI::AdaptiveNamespace::IAdaptiveToggleTarget> target;
        THROW_IF_FAILED(vectorIterator->get_Current(&target));

        ComPtr<AdaptiveNamespace::AdaptiveToggleTarget> adaptiveElement =
            PeekInnards<AdaptiveNamespace::AdaptiveToggleTarget>(target);
        if (adaptiveElement == nullptr)
        {
            return E_INVALIDARG;
        }

        std::shared_ptr<AdaptiveSharedNamespace::ToggleVisibilityTargetElement> sharedTarget;
        RETURN_IF_FAILED(adaptiveElement->GetSharedModel(sharedTarget));
        containedElements.push_back(std::AdaptivePointerCast<AdaptiveSharedNamespace::ToggleVisibilityTargetElement>(sharedTarget));
                                 ...
                                 ABI::AdaptiveNamespace::IAdaptiveActionElement** projectedAction) noexcept try
{
    ...
    case ActionType::ToggleVisibility:
        RETURN_IF_FAILED(MakeAndInitialize<::AdaptiveNamespace::AdaptiveToggleVisibility>(
...
HRESULT GenerateToggleTargetProjection(
    const std::vector<std::shared_ptr<AdaptiveSharedNamespace::ToggleVisibilityTargetElement>>& containedElements,
    ABI::Windows::Foundation::Collections::IVector<ABI::AdaptiveNamespace::AdaptiveToggleTarget*>* projectedParentContainer) noexcept try
{
    for (auto& containedElement : containedElements)
    {
        ComPtr<ABI::AdaptiveNamespace::IAdaptiveToggleTarget> projectedContainedElement;
        RETURN_IF_FAILED(MakeAndInitialize<::AdaptiveNamespace::AdaptiveToggleTarget>(
            &projectedContainedElement,
            std::static_pointer_cast<AdaptiveSharedNamespace::ToggleVisibilityTargetElement>(containedElement)));
